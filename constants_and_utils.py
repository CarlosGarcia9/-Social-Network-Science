...
with open('api-key.txt', 'r') as f:
    ...
    grok_key = lines[2].strip()
    assert len(grok_key) >= 10
    claude_key = lines[3].strip()
    assert len(claude_key) >= 10
...
def get_llm_response(model, messages, savename=None, temp=DEFAULT_TEMPERATURE, verbose=False):
    ...
    if 'gpt' in model:
        ...
        # Claude API Integration using the Anthropic library
        ...
        response = client.messages.create(
            ...
            temperature=temp,
            system=system_message,  # Pass the system message separately
            messages=anthropic_messages,
        )
        print("########### RESPONSE ##################")
        print(response)

        # Save token usage if requested
        if savename is not None:
            ...
            data["prompt_tokens"] += sum(len(m["content"]) for m in anthropic_messages)
            data["completion_tokens"] += len(response.content)
...
def repeat_prompt_until_parsed(model, system_prompt, user_prompt, parse_method,
    ...
    while num_tries <= max_tries:
        try:
            #print("TRY 1111111111111111111111111111111 ###")
            #print(model)
            #print("******************MESSAGES*****************")
            #print(messages)
            #print("******************TEMP*****************")
